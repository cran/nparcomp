npar.t.test <-
function (formula, data, conflevel = 0.05, alternative = c("two.sided", 
    "lower", "greater"), rounds = 3, asy.method = c("logit", 
    "probit", "normal", "t.app"), p.permu = TRUE, plot.simci = TRUE, 
    info = TRUE) 
{
    ssq <- function(x) {
        sum(x * x)
    ...
    a <- nlevels(factorx)
    if (a > 2) {
        stop("You want to perform a contrast test. Please use the function nparcomp")
    }
    ...
    if (p.permu == TRUE) {
        ...
        p.permutation <- p.perm(10000, response, n[1], n[2], 
            t.bf)
    }
    else {
        p.permutation <- NA
    ...
    if (alternative == "two.sided") {
        ...
        text.output.KI <- paste(100 * (1 - conflevel), "%", "2-sided", 
    ...
    if (alternative == "lower") {
        ...
        text.output.p <- "H_0: p(i,j)<=1/2"
        text.output.KI <- paste(100 * (1 - conflevel), "%", "1-sided", 
            "Confidence-Interval for Relative Effect")
        upper <- "]"
        lower <- "("
    }
    ...
    wilcoxon <- data.frame(row.names = 1, Comparison = cmpid, 
        rel.effect = pd, p.value = wilcox.test(response ~ factorx, 
            data, exact = FALSE)$p.value)
    data.info <- data.frame(row.names = 1:2, Sample = fl, Size = n)
    switch(asy.method, logit = {
        x.werte = cbind(lower.logit, pd, upper.logit)
        result <- list(Data.Info = data.info, Analysis.of.relative.effects = data.frame(row.names = 1, 
            Comparison = cmpid, rel.effect = pd, confidence.interval = logit.output, 
            t.value = t.logit, p.value = p.bflogit, p.perm = p.permutation), 
            Wilcoxon.Test = wilcoxon)
        Asymptotic.Method <- " Delta-Method (Logit)"
    }, probit = {
        x.werte = cbind(lower.probit, pd, upper.probit)
        result <- list(Data.Info = data.info, Analysis.of.relative.effects = data.frame(row.names = c(1:nc), 
            Comparison = cmpid, rel.effect = pd, confidence.interval = probit.output, 
            t.value = t.probit, p.value = p.bfprobit, p.perm = p.permutation), 
            Wilcoxon.Test = wilcoxon)
        ...
        result <- list(Data.Info = data.info, Analysis.of.relative.effects = data.frame(row.names = c(1:nc), 
            ...
            t.value = t.bf, p.value = p.bfn, p.perm = p.permutation), 
            Wilcoxon.Test = wilcoxon)
        ...
        result <- list(Data.Info = data.info, Analysis.of.relative.effects = data.frame(row.names = c(1:nc), 
            ...
            t.value = t.bf, p.value = p.bft, p.perm = p.permutation), 
            Wilcoxon.Test = wilcoxon)
        Asymptotic.Method <- paste(" t - Distribution with d.f.= ", 
            round(df.sw, 4))
    })
    if (plot.simci == TRUE) {
        ...
        box()
        title(main = c(text.output.KI, paste("Method:", "", Asymptotic.Method, 
            sep = "")), ylab = "Comparison", xlab = paste("lower", 
            lower, "-----", "p", "------", upper, "upper"))
    }
    if (info == TRUE) {
